...
Product requirements are FR-1 through FR-14 and NFR-1 through NFR-17 from the PRD. Implementation notes are kept separate: Express, Mongoose, EJS, Socket.IO, PM2, route names, schema fields, and dashboard components are constraints and source context, not product goals.
...
Every FR and NFR is mapped to at least one epic/story in section 7. Stories include concrete Given/When/Then acceptance criteria and test notes. No template placeholder remains.
...
## 8. FR/NFR Coverage Map

| Requirement | Covered By |
| --- | --- |
| FR-1 Generate scoped Attendance QR | ST-3.1, ST-3.3 |
| FR-2 Validate Check-in Token | ST-1.3, ST-3.2 |
| FR-3 Resolve Participant for Check-in | ST-2.1, ST-2.2, ST-4.1, ST-4.3 |
| FR-4 Record Attendance State | ST-4.2, ST-4.3 |
| FR-5 Preserve Check-in Event evidence | ST-4.4, ST-9.1, ST-9.3 |
| FR-6 Enforce allowed network ranges for check-in | ST-5.1, ST-5.2 |
| FR-7 Configure network verification surfaces | ST-5.1, ST-5.3 |
| FR-8 Show attendance counts by Conference Code | ST-6.1, ST-6.3 |
| FR-9 Realtime update after successful Check-in | ST-6.2, ST-6.3 |
| FR-10 Admin view and correct Attendance State | ST-7.1, ST-7.2 |
| FR-11 Conference-scoped Participant ID | ST-2.1, ST-2.3, ST-9.2 |
| FR-12 Duplicate and ambiguous identity protection | ST-2.3, ST-4.1, ST-4.3 |
| FR-13 Admin attendance API aligns with UI | ST-7.1, ST-7.2 |
| FR-14 Conference-scoped bulk email or UI removal | ST-8.1, ST-8.2, ST-8.3 |
| NFR-1 Persist before success | ST-4.2, ST-9.2 |
| NFR-2 Token validation works in deployment topology | ST-1.3, ST-3.2 |
| NFR-3 Check-in completes within 2 seconds | ST-4.2, ST-9.4 |
| NFR-4 Admin attendance mutations authenticated and authorized | ST-1.2, ST-7.1 |
| NFR-5 Public surfaces do not expose participant-level attendance | ST-6.4, ST-7.2 |
| NFR-6 Check-in outcomes diagnosable | ST-4.4, ST-9.3 |
| NFR-7 Failure states readable without color only | ST-3.2, ST-4.3, ST-5.2 |
| NFR-8 QR validation authority is server-side | ST-3.2 |
| NFR-9 QR failures do not leak participant data | ST-3.2, ST-4.3 |
| NFR-10 Participant-level attendance remains authenticated | ST-6.4, ST-7.1 |
| NFR-11 Failed check-in states avoid identity leakage | ST-4.3 |
| NFR-12 CSRF addressed before admin attendance production | ST-1.2, ST-7.1 |
| NFR-13 Startup requires explicit MONGODB_URI | ST-1.1 |
| NFR-14 QR URL generation uses correct deployed base URL | ST-1.3, ST-3.1 |
| NFR-15 Seed/admin setup is reliable | ST-1.1 |
| NFR-16 Attendance evidence retained through reporting | ST-4.4, ST-9.1 |
| NFR-17 Export distinguishes registered and checked-in | ST-7.3, ST-9.1 |

## 9. Full Epic Breakdown
...
- Role and CSRF requirements are explicit.
...
- Test valid conference, unknown conference, missing code, and TTL response metadata.
...
- Include performance observation for normal internal load target.
...
- Ensure all messages are readable without color-only cues.
...
- Ensure denial messages are clear and accessible.
...
- Test unauthorized, missing CSRF, wrong conference, success, and duplicate/no-op changes.
...
- Test unauthorized and missing-CSRF behavior.
...
- All FR-1 through FR-14 are mapped in section 8.
- All NFR-1 through NFR-17 are mapped in section 8.
